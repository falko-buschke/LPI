...
output.mat <-  read.table("IterationOutputCurve.txt",header=TRUE,sep="\t", na.strings="NA", dec=".", strip.white=TRUE)
...
png(filename="FigureShape.png",width=14,height=21,units="cm",res=300)
par(mfrow=c(3,2))
...
polygon(c(seq(1970,2020),seq(2020,1970)), 
    c(lpi_50l$CI_low,rev(lpi_50l$CI_high)),col=col1t,border=NA)
# Include error bars as polygons
polygon(c(seq(1970,2020),seq(2020,1970)), 
    c(lpi_100l$CI_low,rev(lpi_100l$CI_high)),col=col2t,border=NA)
# Include error bars as polygons
polygon(c(seq(1970,2020),seq(2020,1970)), 
    c(lpi_150l$CI_low,rev(lpi_150l$CI_high)),col=col3t,border=NA)
...
# Include error bars as polygons
polygon(c(seq(1970,2020),seq(2020,1970)), 
    c(lpi_50h$CI_low,rev(lpi_50h$CI_high)),col=col1t,border=NA)
# Include error bars as polygons
polygon(c(seq(1970,2020),seq(2020,1970)), 
    c(lpi_100h$CI_low,rev(lpi_100h$CI_high)),col=col2t,border=NA)
# Include error bars as polygons
polygon(c(seq(1970,2020),seq(2020,1970)), 
    c(lpi_150h$CI_low,rev(lpi_150h$CI_high)),col=col3t,border=NA)
...
#########################################
plot(0,0,type="n",xlim=c(0.35,0.55), ylim=c(0,50),
	las=1, main="",ylab="Density", xlab="LPI in 2020 (1970 = 1)",
	cex.axis=1.1, cex.lab= 1.3, mgp=c(2.5,0.6,0))

#polygon(density(output.mat[,4],bw=0.007), col=rgb(0,0,0.5,0.4),border=NA)
